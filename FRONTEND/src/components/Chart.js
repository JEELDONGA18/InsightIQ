...
const Chart = ({ type = "line", data = [], labels = [], compact = false }) => {
  ...
  if (!formattedData.length) return <div className="text-cyan-300 text-center p-6">No data available</div>;
  ...
  const chartWrapperStyle = "relative bg-gray-900/90 p-4 rounded-2xl shadow-[0_0_30px_rgba(0,255,255,0.2)] border border-cyan-500/30 overflow-hidden";
        ...
        {type === "line" ? (
          <LineChart data={formattedData}>
            ...
            <Line
              ...
              strokeWidth={3}
              dot={{ r: compact ? 3 : 6, fill: "#00ffff", stroke: "#00bcd4", strokeWidth: 2 }}
              activeDot={{ r: compact ? 4 : 8, fill: "#00e5ff", stroke: "#00bcd4", strokeWidth: 3 }}
          ...
          <PieChart>
            ...
            >
              {formattedData.map((entry, i) => (
                <Cell key={i} fill={`hsl(${180 + i * 15}, 100%, 55%)`} stroke="#00ffff" strokeWidth={1} />
              ))}
            </Pie>
            <Tooltip contentStyle={tooltipStyle} />
